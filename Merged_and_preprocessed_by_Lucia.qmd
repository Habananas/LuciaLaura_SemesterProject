...
```{r}
# library(sf)
all_tracks_LS <- st_as_sf(all_tracks_LS, coords = c("lon", "lat"), crs = 4326, remove = F)
str(all_tracks_LS)
```
...
##  library(sf)
all_tracks_Laura <- st_as_sf(all_tracks_Laura, coords = c("lon", "lat"), crs = 4326, remove=F)
...
```{r}
# Calculation of mean Euclidean distances to positions within the temporal window v
all_tracks <- all_tracks |>
    rowwise() |>
    mutate(
...
To do this, I will use a threshold of 4m stepmean (= mean euclidean distance traveled within the surrounding 10 fixes).
With a sampling frequency of 1s and a constant speed, this would account to a moving speed of 3km/h. 
...
#   perhabs you have to extract the coordinates from all_tracks
...
line <- st_cast(st_union(rec_8), "LINESTRING")

all_tracks|>  filter(record_id== 8) |> group_by(segment_id) |>  
  tm_shape()+
  tm_dots() +
  tm_shape(line) +
  tm_lines()
